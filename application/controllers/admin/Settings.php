    ...
    function index()
    {
        ...
        if ($vals = $this->input->post()) {
            ...
            if (($_FILES["footer_logo_image"]["name"] != "")) {
                $image = upload_file(UPLOAD_PATH . "images/", 'footer_logo_image', 'image', toSlugUrl($vals['site_name']).'-footer-logo');
                if (!empty($image['file_name'])) {
            ...
            if (($_FILES["site_email_logo"]["name"] != "")) {
                $image = upload_file(UPLOAD_PATH . "images/", 'site_email_logo','image',toSlugUrl($vals['site_name']).'-thumb');
                if (!empty($image['file_name'])) {
                    if (!empty($this->data['adminsite_setting']->site_email_logo))
                        $this->remove_file($this->data['adminsite_setting']->site_email_logo);
                    $vals['site_email_logo'] = $image['file_name'];
                } else {
                    setMsg('errorMsg', 'Please upload a valid thumb image file >> ' . strip_tags($image['error']));
                    redirect(ADMIN . '/settings', 'refresh');
                    exit;
